...
/**
 * Which moment the Scheduler brings into view when it is first drawn.
 *
 * A schedule that opens at midnight, or on the first of the month, is a schedule that asks the
 * reader to find the present before they can use it.
 *
 * - `auto` — now, when now is inside the rendered range; otherwise the start of business hours;
 *   otherwise the top of the grid. The one to leave alone.
 * - `now` — the current time, whatever the range. In a range that does not contain today the
 *   time of day is still used, because "the shape of a working day" is what makes it useful.
 * - `business` — the start of `businessHours`.
 * - `none` — nothing is scrolled, and the view opens where the markup puts it.
 * - a number — minutes from midnight. `510` is 08:30.
 * - a string — `"HH:mm"`.
 * - a `Date` — that INSTANT, day included. The keywords and the times of day name a position
 *   inside a day, which is all a day grid needs; an axis that spans a month is several screens
 *   wide, and on it "the 15th at 08:00" is a different place from "08:00". A `Date` is also the
 *   shape an application already holds when what it wants to show is a booking, so bringing it
 *   into view is a binding rather than a call into the component.
 *
 * @group Types
 */
export type SchedulerScrollTime = 'auto' | 'now' | 'business' | 'none' | number | string | Date;
