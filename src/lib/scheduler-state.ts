...
export interface SchedulerStateInputs {
    ...
    scrollTime: Signal<SchedulerScrollTime>;
...
export class SchedulerState {
    ...
    /**
     * Views whose scope is declared inside `<ionic-scheduler-content>`, in declaration order. Written by
    ...
    /** Which moment a view should bring into view when it is drawn. @see SchedulerScrollTime */
    readonly scrollTime = computed(() => this.inputs.scrollTime());

    /**
     * Bumped to ask the views to scroll again, which is what `Scheduler.scrollToTime()` does.
     *
     * A counter and not the target itself: asking twice for the same moment has to move the view
     * twice — the user has scrolled away in between, which is the whole reason they pressed the
     * button.
     */
    readonly scrollRequest = signal(0);

    /**
     * The instant to bring into view, resolved against the rendered range, or `null` for "leave it".
     *
     * Both halves are returned because the views need different things from it: a time grid has one
     * day's worth of axis and cares only about the time of day, while a timeline axis can span a
     * year and cares about the instant. Resolving it once keeps the two from disagreeing.
     */
    readonly scrollTarget = computed<{ minutes: number; instant: Date } | null>(() => {
        const value = this.inputs.scrollTime();

        if (value === 'none') return null;

        // An explicit instant answers for itself: the day is the caller's, not the range's, which is
        // the whole reason to pass one. An invalid `Date` asks for nothing rather than for NaN.
        if (value instanceof Date) {
            return Number.isNaN(value.getTime()) ? null : { minutes: value.getHours() * 60 + value.getMinutes(), instant: value };
        }

        const { start: dayStart } = this.dayBounds();
        const business = this.inputs.businessHours();
        const { start, end } = this.range();
        const now = this.now();
        const inRange = now.getTime() >= start.getTime() && now.getTime() < end.getTime();

        const explicit = parseScrollTime(value);

        if (explicit != null) return { minutes: explicit, instant: atMinutes(inRange ? now : start, explicit) };

        if (value === 'business' || (value === 'auto' && !inRange && business)) {
            const minutes = (business?.start ?? dayStart) * 60;
            return { minutes, instant: atMinutes(start, minutes) };
        }

        // `now`, and `auto` while the range holds it.
        if (value === 'now' || inRange) {
            const minutes = now.getHours() * 60 + now.getMinutes();
            return { minutes, instant: inRange ? now : atMinutes(start, minutes) };
        }

        // `auto`, the range is elsewhere and no business hours were declared: the top of the grid is
        // already where it opens, and scrolling to it would be a no-op with a reflow attached.
        return null;
    });
    ...
    ): { key: string; offset: number; size: number; label: string; ariaLabel: string; full: boolean; slot: SchedulerAppointmentSlot }[] {
    ...
    readonly drag = new SchedulerDragController({
        ...
        snapMinutes: (target: SchedulerDragTarget) =>
            target.whole ? 24 * 60 : timelineScaleOf(this.inputs.view()) ? (this.inputs.timelineSnapDuration() ?? this.inputs.snapDuration()) : this.inputs.snapDuration(),
...
/**
 * Minutes from midnight out of an explicit `scrollTime`, or `null` when the value is one of the
 * keywords.
 *
 * A bare number is minutes; `"08:30"` is the written form. A string of digits is read as minutes
 * too, because that is what a template binding produces when the page writes `scrollTime="510"`.
 */
function parseScrollTime(value: SchedulerScrollTime): number | null {
    if (typeof value === 'number') return Number.isFinite(value) ? clampMinutes(value) : null;
    if (typeof value !== 'string') return null;

    const written = /^(\d{1,2}):(\d{2})$/.exec(value.trim());

    if (written) return clampMinutes(Number(written[1]) * 60 + Number(written[2]));
    if (/^\d+$/.test(value.trim())) return clampMinutes(Number(value));

    return null;
}

/** Keeps a time of day inside the day it belongs to. */
function clampMinutes(minutes: number): number {
    return Math.min(Math.max(Math.round(minutes), 0), 24 * 60);
}

/** A date at a given number of minutes from ITS midnight. */
