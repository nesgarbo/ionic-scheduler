...
import { SCHEDULER_ROOT, type SchedulerRootRef } from './scheduler-root-token';
 ...
 */
@Component({
    selector: 'ionic-scheduler-root',
    standalone: true,
    template: `<ng-content />`,
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    providers: [
        { provide: SCHEDULER_ROOT, useExisting: Scheduler },
        { provide: SCHEDULER_STATE, useFactory: () => inject(Scheduler).schedulerState }
    ],
    host: {
        class: 'ionic-scheduler',
        ...
        '[style.--ionic-scheduler-timeline-slot-width]': 'timelineSlotWidth() != null ? timelineSlotWidth() + "px" : null',
        '[attr.data-density]': 'density()',
        '[attr.data-event-shell]': 'eventShell()',
        '[attr.aria-label]': 'ariaLabel()',
        '[style.--ionic-scheduler-timeline-row-height]': 'resourceRowHeight() != null ? resourceRowHeight() + "px" : null',
        '[attr.data-row-auto-height]': 'rowAutoHeight() ? "" : null',
        '[attr.data-disabled]': 'disabled() ? "" : null',
        '[attr.aria-busy]': 'loading()'
    }
})
export class Scheduler implements SchedulerRootRef {
    ...
    print(options: SchedulerPrintOptions = {}): void {
        ...
        container.id = 'ionic-scheduler-print-root';
        ...
        doc.getElementById('ionic-scheduler-print-root')?.remove();
        doc.body.appendChild(container);
        doc.documentElement.setAttribute('data-ionic-scheduler-printing', '');

        const page = orientation === 'auto' ? null : doc.createElement('style');

        if (page) {
            page.id = 'ionic-scheduler-print-page';
        ...
        const cleanup = () => {
            container.remove();
            doc.documentElement.removeAttribute('data-ionic-scheduler-printing');
    ...
    /**
     * Scrolls the active view back to the moment that matters, or to one you name.
     *
     * The automatic scroll happens once per range, on purpose: a view that jumps back to now while
     * someone is reading last Tuesday is a view fighting its user. This is the other half of that
     * decision — the "now" button on the toolbar, the jump back after an edit — and it moves the
     * view every time it is called, whether or not the target changed.
     *
     * ```ts
     * @ViewChild(Scheduler) scheduler!: Scheduler;
     *
     * goToNow() { this.scheduler.scrollToTime('now'); }
     * ```
     */
    scrollToTime(target?: SchedulerScrollTime): void {
        if (target != null) this.pendingScrollTime.set(target);
        this.schedulerState.scrollRequest.update((count) => count + 1);
    }

    /** @internal A one-off target set by {@link scrollToTime}, until the next input wins. */
    private readonly pendingScrollTime = signal<SchedulerScrollTime | null>(null);

    /** @internal The target in force: whatever the last `scrollToTime()` named, else the input. */
    private readonly resolvedScrollTime = computed<SchedulerScrollTime>(() => this.pendingScrollTime() ?? this.scrollTime());
     ...
     *
     * Left unset, the selector offers the view scopes declared inside `<ionic-scheduler-content>`, which
    ...
    /**
     * Which moment the view brings into view when it is drawn.
     *
     * A day grid opens at midnight and a resource timeline opens on the 1st, and neither is where
     * the work is: `auto` scrolls to now while the range contains it, and to the start of business
     * hours when it does not. Pass `"08:30"`, a number of minutes, or `none` to leave the view where
     * the markup puts it. A `Date` names an instant, day included, which is what an axis spanning a
     * month needs — and being an input, a page that holds that date in a signal brings it into view
     * by binding it rather than by calling {@link scrollToTime}.
     *
     * It runs when the view is first drawn and whenever the range or the view changes — not on every
     * update, because yanking the grid back while someone is reading it is worse than opening in the
     * wrong place.
     * @defaultValue auto
     * @group Props
     */
    readonly scrollTime = input<SchedulerScrollTime>('auto');
     ...
     */
    private readonly resolvedFirstDayOfWeek = computed(() => this.firstDayOfWeek() ?? localeFirstDayOfWeek(this.locale()));
    ...
    readonly schedulerState = new SchedulerState({
        ...
        scrollTime: this.resolvedScrollTime,
 ...
 *
 * Optimus asked its own locale service for this; without one the locale itself is the authority.
 * `Intl.Locale.getWeekInfo()` is the standard answer and numbers the days 1 = Monday to 7 = Sunday,
 * which is not the numbering the rest of the component uses. Safari below 17 and Firefox below 129
 * do not implement it, so the fallback is Sunday — the same default Optimus shipped.
 */
function localeFirstDayOfWeek(locale?: string): number {
    try {
        const info = (new Intl.Locale(locale ?? (typeof navigator !== 'undefined' ? navigator.language : 'en-US')) as any).getWeekInfo?.();
