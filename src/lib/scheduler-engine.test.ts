...
describe('import and export', () => {
    ...
    it('a broken calendar yields what can be read rather than an exception', () => {
        const { events: parsed } = parseICalendar(
...
describe('cell navigation', () => {
    ...
    function grid(html: string): HTMLElement {
        ...
        root.className = 'ionic-scheduler-view';
        root.innerHTML = html;
    ...
    afterEach(() => document.querySelectorAll('.ionic-scheduler-view').forEach((node) => node.remove()));

    it('in a time column the vertical keys walk the column and the horizontal ones change column', () => {
        const root = grid(`
            <div class="ionic-scheduler-time-grid-column">${cell('a1')}${cell('a2')}${cell('a3')}</div>
            <div class="ionic-scheduler-time-grid-column">${cell('b1')}${cell('b2')}${cell('b3')}</div>
        `);
    ...
    it("a mini-month's disabled padding is skipped without losing the weekday column", () => {
        ...
        const root = grid(`<div class="ionic-scheduler-mini-month-grid">${cells7(1)}${cells7(2)}</div>`);
        const cells = [...root.querySelectorAll<HTMLElement>('[data-nav-cell]')];
        const at = (label: string) => cells.find((node) => node.getAttribute('aria-label') === label)!;
    ...
    it('in a month week it is the other way round, and down jumps to the next week', () => {
        const root = grid(`
            <div class="ionic-scheduler-month-week">${cell('w1d1')}${cell('w1d2')}</div>
            <div class="ionic-scheduler-month-week">${cell('w2d1')}${cell('w2d2')}</div>
        `);
        const cells = [...root.querySelectorAll<HTMLElement>('[data-nav-cell]')];
        const at = (label: string) => cells.find((node) => node.getAttribute('aria-label') === label)!;
    ...
    it('a mini-month is a seven-wide grid: down advances a week', () => {
        const root = grid(`<div class="ionic-scheduler-mini-month-grid">${Array.from({ length: 14 }, (_, i) => cell(`d${i + 1}`)).join('')}</div>`);
        const cells = [...root.querySelectorAll<HTMLElement>('[data-nav-cell]')];
    ...
    it('a move that would leave the grid is NOT consumed, so focus is not trapped at the edge', () => {
        const root = grid(`<div class="ionic-scheduler-time-grid-column">${cell('only')}</div>`);
    ...
    it('in RTL the horizontal arrows are reversed', () => {
        const root = grid(`
            <div class="ionic-scheduler-month-week">${cell('d1')}${cell('d2')}</div>
        `);
        const cells = [...root.querySelectorAll<HTMLElement>('[data-nav-cell]')];
    ...
    it('the focus leaves a single tab stop behind it', () => {
        const root = grid(`<div class="ionic-scheduler-month-week">${cell('d1')}${cell('d2')}</div>`);
        const cells = [...root.querySelectorAll<HTMLElement>('[data-nav-cell]')];
