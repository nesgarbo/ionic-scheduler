 ...
 */
@Component({
    selector: 'ionic-scheduler-year-view',
    standalone: true,
    imports: [NgTemplateOutlet],
    template: `
        <div class="ionic-scheduler-year" data-slot="scheduler-year" [attr.data-view]="view">
            @for (month of months(); track month.key) {
                <div class="ionic-scheduler-mini-month" data-slot="scheduler-mini-month">
                    <div class="ionic-scheduler-mini-month-header" data-slot="scheduler-mini-month-header">
                    ...
                    <!-- ONE grid with the weekday names as its first row: in two separate grids the
                         ...
                         numbers. -->
                    <div class="ionic-scheduler-mini-month-grid" role="grid">
                        @for (weekday of weekdayInitials(); track $index) {
                            <span class="ionic-scheduler-mini-month-weekday" aria-hidden="true">{{ weekday }}</span>
                        }
                        @for (day of month.days; track day.key) {
                            <button
                                type="button"
                                class="ionic-scheduler-mini-month-day"
                                data-slot="scheduler-mini-month-day"
                                [class.ionic-scheduler-mini-month-day-other]="day.otherMonth"
                                [class.ionic-scheduler-mini-month-day-weekend]="day.weekend"
                                [class.ionic-scheduler-mini-month-day-has-events]="day.count > 0"
                            ...
                            >
                                @if (miniMonthCellDef(); as tpl) {
                                    ...
                                    <span class="ionic-scheduler-mini-month-number">{{ day.date.getDate() }}</span>
                                    @if (day.count > 0) {
                                        <span class="ionic-scheduler-mini-month-indicator" [style.--ionic-scheduler-mini-month-indicator-background]="day.indicatorColor" aria-hidden="true"></span>
                                    }
                                }
                            </button>
                        }
    ...
    host: { class: 'ionic-scheduler-view ionic-scheduler-view-year' }
})
