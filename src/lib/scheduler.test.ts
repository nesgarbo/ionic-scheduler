...
import type { SchedulerCategory, SchedulerEvent, SchedulerViewType } from '../types/scheduler.types';
import { Scheduler } from './scheduler';
import { SchedulerModule } from './scheduler.module';
import { SchedulerMorePopover } from './scheduler-overlays';
...
@Component({
    // Standalone with the module imported, rather than declared in the testing module: the AOT
    // compiler resolves an inline template against the component's OWN imports, and a class that
    // only reaches its parts through a TestBed declaration has nothing to check the template with.
    ...
    template: `
        ...
        >
            ...
            <ionic-scheduler-category-legend />
            <ionic-scheduler-content>
                <ionic-scheduler-event *schedulerEventDef="let ctx">
                    <span class="fallback-card">{{ ctx.title }}</span>
                </ionic-scheduler-event>
                ...
                <ionic-scheduler-week>
                    <ionic-scheduler-time-grid-event *schedulerTimeGridEventDef="let ctx">
                        <span class="week-card" [attr.data-accent]="ctx.accentColor">{{ ctx.title }}</span>
                    </ionic-scheduler-time-grid-event>
                </ionic-scheduler-week>
                <ionic-scheduler-agenda />
                <ionic-scheduler-day />
                <ionic-scheduler-year />
                <ionic-scheduler-timeline />
                <ionic-scheduler-resource-timeline />
            </ionic-scheduler-content>
            <ionic-scheduler-more-popover />
            <ionic-scheduler-loading />
        </ionic-scheduler-root>
    `
})
class TestHost {
...
describe('Scheduler', () => {
    ...
    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [SchedulerModule, TestHost],
    ...
    it('mounts the compound tree and draws the active view', () => {
        ...
        expect(q('.ionic-scheduler-view-month').length).toBe(1);
    });

    it('the month is 6 weeks of 7 days', () => {
        expect(q('.ionic-scheduler-month-week').length).toBe(6);
    ...
    it("switching to week uses the week's definition and the time grid appears", async () => {
        ...
        expect(q('.ionic-scheduler-view-time-grid').length).toBe(1);
    ...
    it('in the agenda each day is a group and the empty days do not appear', async () => {
        ...
        expect(q('.ionic-scheduler-agenda-empty').length).toBe(0);
    });
    ...
    it('monthCount draws one captioned grid per month and pages by all of them', async () => {
        ...
        expect(q('.ionic-scheduler-month').length).toBe(3);
        expect(q('.ionic-scheduler-month-week').length).toBe(18); // 3 rejillas de 6 semanas
        expect(q('[data-slot="scheduler-month-title"]').length).toBe(3);
        expect(q('.ionic-scheduler-month[data-month="2026-11"]').length).toBe(1);
        ...
        expect(q('.ionic-scheduler-month[data-month="2026-10"] [data-slot="scheduler-month-cell"][data-date="2026-10-01"]:not([data-other-month])').length).toBe(1);
        ...
        expect(q('.ionic-scheduler-month[data-month="2026-12"]').length).toBe(1);
        expect(q('.ionic-scheduler-month[data-month="2026-10"]').length).toBe(0);
    });
    ...
    it('a single month keeps its caption off and its grid unchanged', () => {
        ...
        expect(q('.ionic-scheduler-month').length).toBe(1);
    });
    ...
    it('the "+N more" opens the overflow popover with that day events', async () => {
        // The panel is an ion-popover, which lives in the DOM whether or not it is up: presence
        // proves nothing here and `isOpen` is the state that decides whether Ionic presents it.
        const popover = () => q('[data-slot="scheduler-more-popover-panel"]')[0].nativeElement as HTMLElement & { isOpen: boolean };
        const overlay = () => fixture.debugElement.query(By.directive(SchedulerMorePopover)).componentInstance as SchedulerMorePopover;
        ...
        expect(overlay().context().events.length).toBeGreaterThan(0);
    ...
    it('the "+N more" always reports, and with the popover off it does NOT open the panel', async () => {
        ...
        expect((q('[data-slot="scheduler-more-popover-panel"]')[0].nativeElement as HTMLElement & { isOpen: boolean }).isOpen).toBe(false);
    });
    ...
    it('the year is twelve mini-months and marks the days with events', async () => {
        ...
        expect(q('.ionic-scheduler-mini-month-day-has-events').length).toBeGreaterThan(0);
        // The year is a navigator: pressing a day takes you to the day view.
        const marked = q('.ionic-scheduler-mini-month-day-has-events')[0];
    ...
    it('the resource timeline gives one lane per resource and collects the orphans', async () => {
        ...
        const labels = q('.ionic-scheduler-resource-label').map((el) => (el.nativeElement as HTMLElement).textContent?.trim());
    ...
    it('the now indicator can be turned off, and the density reaches the DOM', async () => {
        ...
        host.view.set('day');
        await fixture.whenStable();
        expect(q('.ionic-scheduler-now-indicator').length).toBe(1);

        host.nowIndicator.set(false);
        await fixture.whenStable();
        expect(q('.ionic-scheduler-now-indicator').length).toBe(0);

        host.density.set('compact');
        await fixture.whenStable();
        expect((q('.ionic-scheduler')[0].nativeElement as HTMLElement).getAttribute('data-density')).toBe('compact');
    });
    ...
    it('the agenda lists empty days only when asked to', async () => {
        ...
        const groups = () => q('.ionic-scheduler-agenda-group').length;
    ...
    it('timeFormat decides the clock the gutter and the events print', async () => {
        ...
        const gutter = text('.ionic-scheduler-time-gutter-slot').filter(Boolean).join(' ');
    ...
    it('print() isolates the schedule and puts the page back afterwards', async () => {
        ...
        window.print = () => {
            ...
            expect(document.documentElement.hasAttribute('data-ionic-scheduler-printing')).toBe(true);

            const container = document.getElementById('ionic-scheduler-print-root');

            expect(container?.parentElement).toBe(document.body);

            const copy = container?.querySelector('.ionic-scheduler[data-printing]') as HTMLElement | null;
            ...
            expect(document.querySelectorAll('.ionic-scheduler[data-printing]').length).toBe(1);
        };
        ...
        expect(document.documentElement.hasAttribute('data-ionic-scheduler-printing')).toBe(false);
        expect(document.getElementById('ionic-scheduler-print-root')).toBeNull();
        expect(document.getElementById('ionic-scheduler-print-page')).toBeNull();
    });
