...
import type { SchedulerAppointmentSlot, SchedulerEvent, SchedulerResource, SchedulerViewType } from '../types/scheduler.types';
 ...
 *
 * The renderers are internal — an application never places `<ionic-scheduler-month-view>`; it declares
 * `<ionic-scheduler-month>` as a *scope* and the root picks the renderer for the active view. Keeping
...
export abstract class SchedulerViewBase {
     ...
     * `<ionic-scheduler-month>` beats one sitting directly in `<ionic-scheduler-content>`.
     */
    ...
    protected bindEvent(
        ...
        extra: Partial<SchedulerEventContext> = {},
        keySuffix?: string
    ): { key: unknown; context: SchedulerEventContext & { $implicit: SchedulerEventContext } } {
    ...
    protected bindCell(
        ...
        extra: Partial<SchedulerCellContext> = {},
        kind = 'cell'
    ): { key: unknown; context: SchedulerCellContext & { $implicit: SchedulerCellContext } } {
