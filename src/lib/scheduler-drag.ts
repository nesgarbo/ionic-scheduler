import type { SchedulerDragPayload, SchedulerDropInfo, SchedulerEvent, SchedulerResizeEdge, SchedulerViewType } from '../types/scheduler.types';
    ...
    private handleMove(originalEvent: PointerEvent): void {
        ...
        if (!session.started) {
            ...
            const payload = this.payload(
                originalEvent,
                { start: new Date(session.baseStart), end: new Date(session.baseEnd), resourceId: session.baseResourceId, allDay: session.baseAllDay },
                session
            );
