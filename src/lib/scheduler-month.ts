 ...
 */
@Component({
    selector: 'ionic-scheduler-month-view',
    standalone: true,
    imports: [NgTemplateOutlet],
    template: `
        @for (panel of panels(); track panel.key) {
            <div class="ionic-scheduler-month" [attr.data-view]="view" [attr.data-grouping]="grouping()" [attr.data-resource-id]="panel.resource?.id" [attr.data-month]="panel.monthKey">
                @if (panel.label) {
                    <!-- One grid per resource has to say whose it is: without the header, three
                     stacked months are three identical months. -->
                    <div class="ionic-scheduler-month-resource-header" data-slot="scheduler-resource-header" [attr.data-resource-id]="panel.resource?.id" [attr.data-event-count]="panel.count">
                        @if (resourceHeaderDef(); as tpl) {
                            <ng-container *ngTemplateOutlet="tpl; context: panel.context" />
                        } @else {
                            @if (panel.resource) {
                                <span class="ionic-scheduler-resource-dot" [style.background]="panel.resource.color" aria-hidden="true"></span>
                            }
                            <span class="ionic-scheduler-resource-label">{{ panel.label }}</span>
                            @if (panel.count) {
                                <span class="ionic-scheduler-resource-count">{{ panel.count }}</span>
                            }
                        }
                    </div>
                }
                @if (panel.monthLabel) {
                    ...
                    <div class="ionic-scheduler-month-title" data-slot="scheduler-month-title" [attr.data-month]="panel.monthKey">
                ...
                <div class="ionic-scheduler-month-header" data-slot="scheduler-month-header-cell">
                    @if (monthHeaderDef(); as tpl) {
                        <ng-container *ngTemplateOutlet="tpl; context: headerContext()" />
                    } @else {
                        @for (weekday of weekdays(); track weekday) {
                            <div class="ionic-scheduler-month-header-cell">{{ weekday }}</div>
                        }
                    }
                </div>

                <div class="ionic-scheduler-month-body">
                    @for (week of panel.weeks; track week.key) {
                        <div class="ionic-scheduler-month-week" [style.--ionic-scheduler-month-rows]="week.rowCount">
                            @for (day of week.days; track day.key) {
                                <div
                                    class="ionic-scheduler-month-cell"
                                    ...
                                    [style.--ionic-scheduler-bar-rows]="day.barRows"
                                ...
                                >
                                    @if (monthCellDef(); as tpl) {
                                        ...
                                        <div class="ionic-scheduler-month-cell-number" data-slot="scheduler-month-cell-number">
                                        ...
                                        <div class="ionic-scheduler-month-day-cell" data-slot="scheduler-month-day-cell">
                                            @if (monthDayCellDef(); as tpl) {
                                                <ng-container *ngTemplateOutlet="tpl; context: day.binding.context; injector: cellInjector(day.binding.key)" />
                                            } @else {
                                                @if (grouping() === 'date') {
                                                    @for (group of day.resourceGroups; track group.key) {
                                                        <span class="ionic-scheduler-month-resource-group" [attr.data-resource-id]="group.resource?.id">
                                                            <span class="ionic-scheduler-event-dot" [style.background]="group.resource?.color" aria-hidden="true"></span>
                                                            {{ group.label }}
                                                        </span>
                                                        @for (item of group.events; track item.key) {
                                                            <button
                                                                type="button"
                                                                class="ionic-scheduler-month-event"
                                                            ...
                                                            >
                                                                @if (monthEventDef(); as tpl) {
                                                                    ...
                                                                    <span class="ionic-scheduler-event-time">{{ item.shortTime }}</span>
                                                                }
                                                            </button>
                                                        }
                                                    }
                                                    ...
                                                    @for (item of day.visibleEvents; track item.key) {
                                                        ...
                                                        >
                                                            @if (monthEventDef(); as tpl) {
                                                                ...
                                                                <span class="ionic-scheduler-event-dot" [style.background]="item.context.accentColor" aria-hidden="true"></span>
                                                                <span class="ionic-scheduler-event-title">{{ item.context.title }}</span>
                                                                <span class="ionic-scheduler-event-time">{{ item.shortTime }}</span>
                                                            }
                                                        </button>
                                        ...
                                        @if (day.overflow > 0) {
                                            <button
                                                type="button"
                                                class="ionic-scheduler-month-more-link"
                                                data-slot="scheduler-month-more-link"
                                                [attr.data-event-count]="day.overflow"
                                                (click)="openMore($event, day.date, day.events)"
                                            >
                            ...
                            @for (item of week.events; track item.key) {
                                <div
                                    class="ionic-scheduler-month-bar"
                                    data-slot="scheduler-month-event"
                                    tabindex="0"
                                    role="button"
                                    data-all-day=""
    ...
    host: { class: 'ionic-scheduler-view ionic-scheduler-view-month', '[attr.data-month-count]': 'state.monthCount()' }
})
