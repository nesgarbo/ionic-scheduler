...
const CONTAINERS: { selector: string; axis: CellAxis; columns?: number }[] = [
    { selector: '.ionic-scheduler-time-grid-column', axis: 'column' },
    { selector: '.ionic-scheduler-all-day-lanes', axis: 'row' },
    { selector: '.ionic-scheduler-month-week', axis: 'row' },
    { selector: '.ionic-scheduler-timeline-cells', axis: 'row' },
    { selector: '.ionic-scheduler-mini-month-grid', axis: 'grid', columns: 7 }
];
...
function targetFor(cell: HTMLElement, key: string, rtl: boolean): HTMLElement | null {
    ...
    const root = cell.closest<HTMLElement>('.ionic-scheduler-view') ?? cell.ownerDocument.body;
