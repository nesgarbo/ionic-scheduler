 ...
 */
@Component({
    selector: 'ionic-scheduler-agenda-view',
    standalone: true,
    imports: [NgTemplateOutlet],
    template: `
        <div class="ionic-scheduler-agenda" data-slot="scheduler-agenda" [attr.data-view]="view">
            @for (group of groups(); track group.key) {
                <div class="ionic-scheduler-agenda-group">
                    <div
                        class="ionic-scheduler-agenda-date-header"
                        data-slot="scheduler-agenda-date-header"
                        [attr.data-date]="group.key"
                        [attr.data-today]="group.today ? '' : null"
                    ...
                    >
                        @if (dateHeaderDef(); as tpl) {
                            ...
                            <span class="ionic-scheduler-agenda-heading">
                                <span class="ionic-scheduler-agenda-day">{{ group.context.dayName }}</span>
                                <span class="ionic-scheduler-agenda-date">{{ group.context.formattedDate }}</span>
                            </span>
                            <span class="ionic-scheduler-agenda-count">{{ group.count }}</span>
                        }
                    </div>
                    ...
                    @for (item of group.events; track item.key) {
                        ...
                        <div class="ionic-scheduler-agenda-row">
                            <div class="ionic-scheduler-agenda-row-gutter" aria-hidden="true"></div>
                            <div
                                class="ionic-scheduler-agenda-event"
                            ...
                            >
                                @if (agendaEventDef(); as tpl) {
                                    ...
                                    <span class="ionic-scheduler-agenda-event-time">{{ item.context.timeText }}</span>
                                }
                            </div>
                        </div>
                    }
                ...
                <div class="ionic-scheduler-agenda-empty">{{ state.labels().empty }}</div>
            }
        </div>
    `,
    changeDetection: ChangeDetectionStrategy.OnPush,
    encapsulation: ViewEncapsulation.None,
    host: { class: 'ionic-scheduler-view ionic-scheduler-view-agenda' }
})
